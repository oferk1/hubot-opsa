...
(function () {
  var AnomAPI, OpsaSessionHandler, Properties, RegistrationHandler, createJar, generateJar, getLabels, getLinkToHost, getOneHourAgoTS, getOpsaUri, getRequestedAnomaliyType, getRequestedHost, getSessionId, hubotRouter, now, progressCallback, registeredListeners, request, requestp;
  ...
  AnomAPI.prototype.invoke = function (userRes) {
    ...
    }).then((function (anomRes) {
      return self.replyWithMetrics(anomRes, userRes);
    }));
  };
  ...
  AnomAPI.prototype.getMetrics = function (anom) {
  ...
  AnomAPI.prototype.replyWithMetrics = function (anomRes, userRes) {
    var anom, anoms, getMetricsAndReply, i, len, results, self;
    anoms = this.parseRes(anomRes, userRes);
    if (anoms.length === 0) {
    ...
    getMetricsAndReply = function (anom) {
      ...
      return function () {
        return self.getMetrics(clonedAnom).then((function (resultRes) {
